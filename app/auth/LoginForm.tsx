...
export default function LoginForm({ onToggle }: LoginFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // แสดงหน้าโหลดโลโก้เป็นเวลา 1.8 วินาทีก่อนเปลี่ยนหน้าไป /dashboard
    setTimeout(() => {
    ...
    <>
      {isLoading && (
        <LogoLoadingScreen 
          ...
          subMessage="ยินดีต้อนรับสู่ระบบ PREMIER Automation CRM" 
        />
      )}
      ...
      <div className="w-full">
        <div className="text-center mb-8 flex flex-col items-center">
          ...
          <img 
            ...
            className="w-16 h-16 object-contain rounded-xl mb-4 shadow-lg border border-white/10" 
          />
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button 
            type="submit" 
            disabled={isLoading}
            className="w-full bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-semibold rounded-xl px-4 py-3.5 mt-2 transition-all hover:-translate-y-0.5 hover:shadow-[0_10px_20px_-10px_rgba(99,102,241,0.5)] active:translate-y-0 flex items-center justify-center gap-2"
          >
