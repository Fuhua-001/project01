...
```tsx
const ITEMS_PER_PAGE = 10; // กำหนดให้ 1 หน้ามีสินค้าได้สูงสุด 10 รายการ (หากเกิน 10 รายการจะตัดขึ้นหน้าใหม่อัตโนมัติ)
const pages = [];
...
- **`items.slice(start, end)`**: คำสั่งตัดแบ่ง Array ตามจำนวนรายการที่ต้องการ เช่น 
  - รอบแรก `items.slice(0, 10)` -> ได้สินค้าชิ้นที่ 1-10 (หน้า 1)
  - รอบสอง `items.slice(10, 20)` -> ได้สินค้าชิ้นที่ 11-20 (หน้า 2)
